...
\usepackage{bm}                                             % 放在 unicode-math 之后作为兜底
...
\usepackage{environ} % 用于捕获环境内容
\usepackage{etoolbox} % 用于设置开关

% --- 定义显示/隐藏开关 ---
\newbool{showsolution}
\boolfalse{showsolution}  % 设置为 true 显示，false 隐藏
...
\definecolor{amzgreen}{RGB}{0, 136, 80}   % 推论使用的绿色
...
\newtcbtheorem[number within=chapter]{corollary}{推论}{
    commonstyle,
    colframe=amzgreen,
    boxed title style={colback=amzgreen, colframe=amzgreen},
}{cor}
...
% \usepackage{etoolbox}
% \makeatletter
% % 增加大算子（operator）上下标的限位间距
% \patchcmd{\slimits@}{\displaystyle}{\displaystyle\addtolength{\jot}{3pt}}{}{}
% \makeatother

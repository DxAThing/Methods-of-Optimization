...
\begin{definition}
    ...
    （1）{\bf 向量的内积运算}：$\bs{x}^{\top} \bs{y} = \bs{y}^{\top} \bs{x} = \displaystyle \sum_{i=1}^n x_iy_i$；

    （2）{\bf 向量长度}：$\| \bs{x} \| = \sqrt{\bs{x}^{\top} \bs{x}} = \sqrt{\displaystyle \sum_{i=1}^n x_i^2}$；

    （3）{\bf 两点间距离}：$\| \bs{x} - \bs{y} \|$. 
\end{definition}

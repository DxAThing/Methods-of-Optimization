...
\begin{theorem}{子空间投影定理}{}

    设$L$为$\R^n$的子空间，则$\forall \bs{z} \in \R^n$，则必有以下两个命题成立：

    \begin{enumerate}
        \item $\exists! \bs{x}\in L, \bs{y} \in L^{\bot}$，使得$\bs{z} = \bs{x}+\bs{y}$；
        \item 在（1）中的$\bs{x}$为问题
    \begin{align*}
...
\begin{align*}
    ...
    = & \bs{0}
\end{align*}
$\forall \bs{\beta}\in\R^m$都成立，所以$\bs{z} - \bs{A}(\bs{A}^{\bot}\bs{A})^{-1}\bs{A}^{\top}\bs{z} \in L^{\top}$，也即取$y = \bs{z} - \bs{A}(\bs{A}^{\top}\bs{A})^{-1}\bs{A}^{\top}\bs{z} \in L^{bot}$即可. 
存在性证毕. 

...
\prove
由定理1.7知，$\text{cov}(S) \subseteq S$，下证$S \subseteq \text{cov}(S)$. 
...
\begin{definition}
    ...
    设集合$S \subseteq \R^n$为凸集，函数$f: S \to R$. 
    ...
    当$-f(\bs{x})$为凸函数（严格凸函数时），则称$f(\bs{x})$为{\bf 凹函数}（{\bf 严格凸函数}）. 
\end{definition}
...
\begin{theorem}
    ...
    $$
        f(\sum_{j=1}^m \lambda_j \bs{x}^{(j)}) \le \sum_{j=1}^m \lambda_j f(\bs{x}^{(j)})
    $$
\end{theorem}
...
\begin{theorem}
    ...
    （1）若$f$是凸函数，则$f$在$\text{int}(S)$上连续；

    （2）若$f$是凸函数，则其对任意方向的方向导数（若方向可行）存在；

    （3）设$S$是开集
    \footnote{
        设$S \in \R^n$，若$\text{int}(S) = S$，则称$S$是{\bf 开集}. 
    }
    ，$f$在$S$上可微，则$f$是凸函数的充要条件是$\forall \bs{x}^* \in S$，都有
    $$
        f(\bs{x}) \ge f(\bs{x}^*) + \nabla f^{\top} (\bs{x}^*) (\bs{x} - \bs{x}^*)
    $$
    $\forall x \in S$都成立；

    （4）设$S$是开集，$f$在$S$上二次可微，则：
    
    （i）$f$是凸函数的充要条件是$\forall \bs{x} \in S$，$\nabla^2 f(\bs{x})$是半正定的；

    （ii）若$\forall \bs{x} \in S$，$\nabla^2 f(\bs{x})$是正定的，则$f$是严格凸函数. 
\end{theorem}
...
\begin{definition}
    ...
    （1）若$(f\ S)$中的$S$为凸集，$f$是$S$上的凸函数，优化目标为$\min$时，则称$(f\ S)$为凸规划；

    （2）若$(fgh)$中的$f, g_i$为凸函数，$h_j$为线性函数，则称$(fgh)$为凸规划. 
\end{definition}

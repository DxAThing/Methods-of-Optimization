...
\begin{enumerate}
    \item 集合\(S = \{ \bs{x} \mid \bs{Ax} = \bs{b} \}\)是凸集；
    \item 集合\(S = \{ \bs{x} \mid \bs{Ax} \le \bs{b} \}\)是凸集. 
\end{enumerate}
...
设\(f_1, f_2\)是凸函数，判断：
\begin{enumerate}
    \item 设\(\lambda_1, \lambda_2 > 0\)，\(\lambda_1 f_1 + \lambda_2 f_2\)是否为凸函数？
    \(\lambda_1 f_1 - \lambda_2 f_2\)是否为凸函数？
    \item \(f(\bs{x}) = \max \{ f_1(\bs{x}), f_2(\bs{x}) \}\)是否为凸函数？
...
\begin{proof}
    ...
    \footnote{
        ...
        由上述定理，正方体（或任意有界凸多面体）实际上是其顶点集的凸包. 之后应用Jensen不等式进行证明即可. 
    }

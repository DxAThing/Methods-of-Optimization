...
\begin{definition}
    ...
    设$S \subseteq \R^n$，$\bs{d} \in \R^n$，若$\bs{d}$是$S$的可行方向（见{\bf 方向导数}的定义），且不能被表示为$S$的两个不同可行方向
    \footnote{
        设$\bs{d}^{(1)}, \bs{d}^{(2)} \in \R^n$，若$\exists \lambda > 0$，使得$\bs{d}^{(1)} = \lambda \bs{d}^{(2)}$，则称$\bs{d}^{(1)}, \bs{d}^{(2)}$是{\bf 同方向}的. 
    }
    的非负组合，则称$\bs{d}$是$S$的{\bf 极方向}. 
\end{definition}
...
\begin{theorem}
    ...
    （ii）$\bs{x} = (\bs{x_B}^{\top}, \bs{x_N}^{\top})$（分解方式与$\bs{A}$相同），其中$\bs{x_B} = \bs{B}^{-1} \bs{b} \ge \bs{0}$，$\bs{x_N} = \bs{0}$. 
...
\begin{theorem}
    ...
    （1）$\bs{d}$是$S$的方向的充要条件是$\bs{Ad} = \bs{0}$且$\bs{d} \ge 0$；

    （2）$\bs{d}$是$S$的极方向的充要条件是存在分解$\bs{A} = (\bs{B}, \bs{N})$，使得
    
    {
        \setlength{\parindent}{4em}
        （i）$\bs{B}$为$m$阶可逆阵；

        （ii）$\bs{d} = (\bs{d_B}^{\top}, \bs{d_N}^{\top})$（分解方式与$\bs{A}$相同），其中$\bs{d_B} = - \bs{B}^{-1} \bs{a}_j \ge \bs{0}$，$\bs{d_N} = \bs{e}_j$. 
...
\begin{theorem}
    ...
    设多面体$S = \{ \bs{x} \in \R^n \mid \bs{Ax} = \bs{b}, \bs{x} \ge \bs{0} \}$ ，其中  $\bs{A} \in \R^{m \times n}$行满秩，$\bs{x}^{(1)}, \bs{x}^{(2)}, \cdots, \bs{x}^{(k)}$为所有极点，$\bs{d}^{(1)}, \bs{d}^{(2)}, \cdots, \bs{d}^{(l)}$为所有极方向，则$\forall \bs{x} \in S$，$\exists \lambda_i >0$，$i = 1, 2, \cdots, k$，且$\displaystyle \sum_{i=1}^k \lambda_i = 1$，$\mu_j \ge 0$，$j = 1, 2, \cdots, l$，使得
    \begin{align*}
        \bs{x} = & \lambda_1 \bs{x}^{(1)} + \lambda_2 \bs{x}^{(2)} + \cdots + \lambda_k \bs{x}^{(k)} & \text{（极点的凸组合）} \\
        + & \mu_1 \bs{x}^{(1)} + \mu_2 \bs{x}^{(2)} + \cdots + \mu_l \bs{x}^{(l)} & \text{（极方向的正组合）}
    \end{align*}
\end{theorem}

...
\begin{definition}{线性规划的一般形式}{}
    ...
    \end{align*}
    其中，称 \( c_i \) 为\textbf{目标函数系数}或\textbf{价值系数}或\textbf{费用系数}；
    称 \( b_j \) 为\textbf{右端项}或\textbf{资源常数}；
    称 \( a_{ij} \) 为\textbf{约束系数}或\textbf{技术系数}. 
\end{definition}
...
为了研究问题的方便，我们将其写成矩阵形式
\begin{align*}
    \max & \quad z = \bs{c}^{\top} \bs{x} \\
    \text{s.t.} & \quad \bs{Ax} = \bs{b} \\
    & \quad \bs{x} \ge \bs{0}
\end{align*}
其中，\(\bs{c}, \bs{x} \in \R^n\)，\(\bs{b} \ge \bs{0}\)，\(\bs{A} \in \R^{m \times n}\). 
线性规划的标准形式记作(LP).

\begin{definition}{线性规划的规范形式}{}
    线性规划的规划形式如下：
    \begin{align*}
    \max & \quad z = \bs{c}^{\top} \bs{x} \\
...
\begin{enumerate}
    \item \( \begin{aligned}[t]
        \min & \quad f = 3.6 x_1 - 5.2 x_2 + 1.8 x_3 \\
        \text{s.t.} & \quad 2.3x_1 + 5.2x_2 - 6.1x_3 \le 15.7 \\
        & \quad  4.1 x_1 + 3.3 x_3 \ge 8.9 \\
        & \quad x_1 + x_2 + x_3 = 38 \\
        & \quad x_1, x_2, x_3 \ge 0
    \end{aligned} \)
    \item \( \begin{aligned}[t]
        \min & \quad f = - 3 x_1 + 5 x_2 + 8 x_3 - 7 x_4\\
        \text{s.t.} & \quad 2x_1 - 3x_2 + 5x_3 + 6x_4 \leq 28 \\
        & \quad 4x_1 + 2x_2 + 3x_3 - 9x_4 \geq 39 \\
        & \quad 6x_2 + 2x_3 + 3x_4 \leq -58 \\
        & \quad x_1, x_2, x_3 \ge 0
    \end{aligned} \)
\end{enumerate}
